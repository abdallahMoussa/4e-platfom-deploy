...
import { useNavigate } from "react-router";
 import AddServiceCatalogPopup from "../popup/AddServiceCatalogPopup";
import EditServiceCatalogPopup from "../popup/EditServiceCatalogPopup";
import EditBasicClassificationPopup from "../popup/EditBasicClassificationPopup";

type ActionProps = {
  id: Number;
};

export const MainClassificationActions: FC<ActionProps> = ({ id }) => {
  const intl = useIntl();
  let navigate = useNavigate();
  const [showDelete, setshowDelete] = useState<boolean>(false);
  const [showEditBasicClassification, setshowEditBasicClassification] = useState<boolean>(false);
  ...
  return (
    <>
        ...
        >
          <div className="menu-item px-3 ">
            ...
            >
              ...
              {intl.formatMessage({ id: "AddServiceCatalog" })}
            </a>
          </div>
          ...
          <div className="menu-item px-3 ">
            <a
              ...
              onClick={(event) => {
                event.preventDefault();
                setshowEditBasicClassification(true)
              }}
            >
      ...
      <AddServiceCatalogPopup  handleClose={() => setshowServiceCatalog(false)}
          show={showServiceCatalog}/>
      <EditBasicClassificationPopup
       handleClose={() => setshowEditBasicClassification(false)}
          show={showEditBasicClassification}
          />
    </>
  );
};

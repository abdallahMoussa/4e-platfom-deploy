...
const WorkspaceCard: FC<Iprop> = ({ id, name, dept, title, subTitle, count, lastUpdate, lastLogin, link, icon, subIcon, color, background, isShow }) => {
  ...
  return (
    <>
      {isShow && (
        <div id={id} className="col-md-4 col-xl-3 col-xxl-3">
          <div className="card">
            ...
            <div className="card-body d-flex flex-center flex-column p-5">
                ...
                >
                  <span
                    className={
                      ...
                      (color === "svg-icon-warning" ? "svg-icon-warning" : color === "svg-icon-primary" ? "svg-icon-primary" : color === "svg-icon-info" ? "svg-icon-info" : color === "svg-icon-gblue" ? "svg-icon-gblue" : color === "svg-icon-success" ? "svg-icon-success" : color === "svg-icon-danger" ? "svg-icon-danger" : "")
                    }
                  >
                    {icon}
                  </span>
                </span>
                <span className="top-icon">{subIcon}</span>
              </div>
              <Link to={link} className="fs-4 text-gray-800 text-hover-gblue fw-bolder mb-0">
                {title}
              </Link>
              <div className="fw-bold text-gray-400 mb-6">{subTitle}</div>
              ...
              <Link to={link} className="btn btn-primary py-3 mx-8 mt-2">

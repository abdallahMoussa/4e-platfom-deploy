...
const WorkspaceTable: FC<Iprop> = ({ name, dept, title, count, lastUpdate, lastLogin, link, icon, color, background, isShow }) => {
  const intl = useIntl();
  return (
    <>
      {isShow && (
        <tr>
            ...
            <div className="d-flex align-items-center">
              <div className="me-5 position-relative">
                <div className="symbol symbol-50px">
                  <span className={"symbol-label " + (background === "bg-light-warning" ? "bg-light-warning" : background === "bg-light-primary" ? "bg-light-primary" : background === "bg-light-info" ? "bg-light-info" : background === "bg-light-gblue" ? "bg-light-gblue" : background === "bg-light-success" ? "bg-light-success" : background === "bg-light-danger" ?"bg-light-danger":"")}>
                    <span className={"svg-icon svg-icon-3x " + (color === "svg-icon-warning" ? "svg-icon-warning" : color === "svg-icon-primary" ? "svg-icon-primary" : color === "svg-icon-info" ? "svg-icon-info" : color === "svg-icon-gblue" ? "svg-icon-gblue" : color === "svg-icon-success" ? "svg-icon-success" : color === "svg-icon-danger"?"svg-icon-danger":"")}>{icon}</span>
                  </span>
                </div>
              </div>
              <div className="d-flex flex-column justify-content-center">
                <Link to={link} className="mb-1 text-gray-800 text-hover-info">
                  {title}
                </Link>
          ...
          <td className="text-end">
            <Link to={link} className="btn btn-light btn-sm me-2">

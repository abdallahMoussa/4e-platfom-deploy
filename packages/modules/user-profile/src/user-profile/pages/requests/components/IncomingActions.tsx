...
export const IncomingActions: FC<ActionProps> = ({ id, item }) => {
  const intl = useIntl();
      ...
      <div className="card-toolbar text-center">
        <div>
          ...
          >
            ...
            {item?.status == "تحت الدراسه" ? (
              <div className="menu-item px-3 " style={{ width: "fit-content" }}>
                <Link
                  to={`/user-profile/requests/${id}/posting-request`}
                  className="menu-link px-3 "
                  state={{
                    type: intl.formatMessage({
                      id: "IncomingRequest",
                ...
                >
                  <i className="bi bi-bag-check mx-2  fs-2"></i>
                  {intl.formatMessage({ id: "TakeAction" })}
                </Link>
              </div>
            ) : (
              ""

...
export const SystemPagination: FC<Iprop> = ({totalPages = 1, page = 1}) => {
  ...
  const updatePageNumber = (newPage: string) => {
    if (searchParams) {
      ...
      let currentPage = searchParams.get('pageIndex')
      ...
      searchParams.set('pageIndex', currentPage)
    ...
    <div className='d-flex flex-stack flex-wrap pt-5'>
      ...
      <ul className='pagination pagination-outline'>
        ...
        {Array.from({length: totalPages}).map((_, index) => (
          <li
            key={index}
            className={`page-item cursor-pointer m-1 ${page === index + 1 && searchParams?.get('pageIndex') === `${index + 1}` ? 'active text-hover-white' : ''}`}
            onClick={() => {

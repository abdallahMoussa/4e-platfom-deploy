...
let baseUserUrl = process.env.REACT_APP_BASE_AGG_URL;

const aggAxiosInstance = axios.create({
...
aggAxiosInstance.interceptors.request.use(
  function (config) {
...
aggAxiosInstance.interceptors.response.use(
  function (response) {
...
export default aggAxiosInstance;

...
const CLoseProjectPopup: FC<Props> = ({ onClose })  => {
  ...
  return (
    <>
      <div className='modal fade' id='Rega_modal-close-project' aria-hidden='true'>
              ...
              <div className='card-body mx-5'>
                <div className='card-header px-1 d-flex justify-content-center  pb-2 mb-5'>
                  <h3 className='card-title flex-column align-items-center'>
                    ...
                    {intl.formatMessage({ id: 'AddReason' })}                      </span>
              
                    <span className='mt-1 fw-semibold fs-5 text-muted'>
                    {intl.formatMessage({ id: 'PleaseWriteTheReason' })}                   </span>
                  </h3>
            
     
            </div>
            <label className='form-label px-2  fontSz-6 mb-5'>  {intl.formatMessage({ id: 'WriteTheReasonForCanceling/SuspensingTheProject' })}</label>
            <textarea className="form-control form-control-solid" rows={5}  placeholder="اضف اسبابك "></textarea>
